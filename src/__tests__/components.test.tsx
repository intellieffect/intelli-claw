...
describe("SessionSwitcher", () => {
  const mockSessions: GatewaySession[] = [
    { key: "agent:karajan:main", updatedAt: 300, label: "카라얀 메인" },
    { key: "agent:brxce:main", updatedAt: 200 },
    { key: "agent:karajan:main:thread:123", updatedAt: 100, totalTokens: 5000 },
  ];
  ...
  it("renders trigger button with current session name", () => {
    render(
      <SessionSwitcher
        ...
        currentKey="agent:karajan:main"
        onSelect={() => {}}
        onNew={() => {}}
      />
    );
    expect(screen.getByText("카라얀 메인")).toBeInTheDocument();
  });
  ...
  it("shows sessions sorted by updatedAt in palette", () => {
    ...
    // Session with updatedAt: 300 should appear (label: 카라얀 메인)
    expect(screen.getByText("카라얀 메인")).toBeInTheDocument();
  ...
  it("calls onSelect when clicking a session", () => {
    ...
    fireEvent.click(screen.getByText("카라얀 메인"));
    expect(onSelect).toHaveBeenCalledWith("agent:karajan:main");
  ...
  it("shows agent badges for each session", () => {
    ...
    const karajanBadges = screen.getAllByText("karajan");
    expect(karajanBadges.length).toBeGreaterThanOrEqual(2); // 2 karajan sessions
