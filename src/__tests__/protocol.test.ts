...
describe("protocol", () => {
    ...
    it("HelloOkPayload has full structure with server, features, snapshot, policy", () => {
      const payload: HelloOkPayload = {
        ...
        snapshot: {
          ...
          uptimeMs: 86400000,
          sessionDefaults: {
            defaultAgentId: "karajan",
            mainKey: "agent:karajan:main",
            mainSessionKey: "agent:karajan:main:thread:123",
          },
          authMode: "token",
        },
      ...
      expect(payload.snapshot.sessionDefaults?.mainSessionKey).toBe("agent:karajan:main:thread:123");
  ...
  describe("chat.send params (Protocol v3)", () => {
    it("ChatSendParams requires sessionKey, message, idempotencyKey", () => {
      const params: ChatSendParams = {
        sessionKey: "agent:karajan:main:thread:123",
  ...
  describe("chat.history params (Protocol v3)", () => {
    it("ChatHistoryParams requires sessionKey with optional limit", () => {
      ...
      expect(params.sessionKey).toBe("agent:karajan:main");
