  ...
  const handleDelete = useCallback(
    (key: string) => {
      Alert.alert("세션 삭제", "이 세션을 삭제하시겠습니까?", [
        ...
        { text: "삭제", style: "destructive", onPress: () => onDelete?.(key) },
      ]);
    },
    [onDelete],
  );
  ...
  const handleReset = useCallback(
    (key: string) => {
      Alert.alert("세션 리셋", "모든 메시지가 초기화됩니다.", [
        { text: "취소", style: "cancel" },
        { text: "리셋", style: "destructive", onPress: () => onReset?.(key) },
      ]);
    },
    [onReset],
  );
  ...
  return (
    <Actionsheet isOpen={visible} onClose={onClose}>
      ...
      <ActionsheetContent className="max-h-[85%]">
        ...
        {/* Header */}
        <View className="flex-row items-center px-5 pb-3.5 border-b border-border gap-3">
          <Text className="text-lg font-bold text-foreground">세션 선택</Text>
        ...
        {/* Search */}
        <View className="flex-row items-center px-5 py-3 border-b border-border/50 gap-3">
        ...
        <SectionList
          ...
          ListHeaderComponent={
            <Pressable
              className={`flex-row items-center px-5 py-4 border-b border-border/50 gap-3 ${!currentKey ? "bg-secondary" : ""}`}
              onPress={() => handleSelect(null)}
              accessibilityLabel="기본 세션"
              accessibilityRole="button"
            >
              <View className="w-8 items-center">
                <Bot size={18} color="hsl(18, 100%, 56%)" />
              </View>
              <View className="flex-1 mr-2">
                <Text className="text-base font-semibold text-foreground">기본 세션 (auto)</Text>
                <Text className="text-sm text-muted-foreground mt-0.5" numberOfLines={1}>
                  {mainSessionKey || "main"}
                </Text>
              </View>
              {!currentKey && <Check size={18} color="#10B981" />}
            </Pressable>
          }
          ListEmptyComponent={
            search ? (
              <View className="py-10 items-center">
                <Text className="text-base text-muted-foreground">검색 결과 없음</Text>
              </View>
            ) : null
          }
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable
                className="flex-row items-center px-5 py-4 border-b border-border/50 gap-3"
                ...
                onLongPress={() => {
                  ...
                  if (actions.length > 0) {
                    Alert.alert(item.title, item.key, [{ text: "취소", style: "cancel" }, ...actions]);
                  }
                }}
              >
                <View className="w-8 items-center">
                ...
                <View className="flex-1 mr-2">
                  ...
                  <View className="flex-row items-center gap-2 mt-1">
                    <View className="px-2 py-0.5 rounded-md" style={{ backgroundColor: color + "20" }}>
                      <Text className="text-xs font-bold" style={{ color }}>{item.agentId}</Text>
                    </View>
                    {item.lastMessage ? (
                ...
                <View className="items-end gap-1">
                  <Text className="text-xs text-muted-foreground/60">{timeAgo(item.updatedAt)}</Text>
        ...
        {/* New session button */}
        <Pressable
          className="flex-row items-center justify-center gap-2.5 py-4 border-t border-border bg-background active:bg-secondary"
          onPress={() => handleSelect(null)}
          accessibilityLabel="새 대화 시작"
          accessibilityRole="button"
        >
          <Plus size={20} color="hsl(217, 91%, 60%)" />
          <Text className="text-base font-semibold text-primary">새 대화 시작</Text>
        </Pressable>
      </ActionsheetContent>
    </Actionsheet>
  );
